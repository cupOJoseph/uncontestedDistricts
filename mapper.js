$(document).ready(function() {
  const map = L.map('mapid').setView([37.5, -79], 7);

  L.tileLayer('https://api.mapbox.com/styles/v1/schiarizzi/cj5fnev7v2gth2sp9qjrcls5h/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1Ijoic2NoaWFyaXp6aSIsImEiOiJjajE4a3NuZWowNzQ5MzNvN2xkdGh2YnVwIn0.dOZQgGCs8Fwxpy7bmRvvTg', {
    id: 'mapbox.light'
  }).addTo(map);
...
`;
  ...
  const replist = [4,5,6,14,15,16,19,22,24,76,78,61];
//list of districts by number where democrats are not running

  const demlist = [11,35,36,37,39,41,43,44,45,46,47,48,52,57,63,70,71,75,79,80,90,92,95 ];
